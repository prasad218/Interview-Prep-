      ...
      <div className="max-w-4xl mx-auto px-4 py-8 sm:py-10">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-ink-500">Payment code redemptions</p>
            </div>
          </div>
          <button
            onClick={handleLogout}
            className="text-xs font-medium text-ink-300 hover:text-ink-100 border border-base-600 hover:border-accent-dim rounded-lg px-3 py-1.5 transition-colors"
          >
            Log out
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
          <div className="rounded-2xl border border-base-600 bg-base-900 shadow-card p-4">
            <p className="text-2xl font-display font-bold text-ink-100">{summary.total}</p>
            <p className="text-xs text-ink-500 mt-0.5">Codes generated</p>
          </div>
          <div className="rounded-2xl border border-base-600 bg-base-900 shadow-card p-4">
            <p className="text-2xl font-display font-bold text-signal-teal">{summary.redeemed}</p>
            <p className="text-xs text-ink-500 mt-0.5">Redeemed</p>
          </div>
          <div className="rounded-2xl border border-base-600 bg-base-900 shadow-card p-4">
            <p className="text-2xl font-display font-bold text-signal-amber">{summary.unredeemed}</p>
            <p className="text-xs text-ink-500 mt-0.5">Waiting to be redeemed</p>
          </div>
        </div>

        <div className="rounded-2xl border border-accent/30 bg-brand-gradient-soft p-4 mb-6">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <button
              ...
              disabled={generating}
              className="rounded-xl bg-brand-gradient hover:opacity-90 shadow-glow-sm disabled:opacity-50 transition-opacity px-4 py-2 text-sm font-semibold text-white whitespace-nowrap"
            >
              {generating ? "Generating…" : "Generate new code"}
            </button>
          </div>
          {newCode && (
            <div className="mt-3 flex items-center gap-2 bg-base-900 border border-base-600 rounded-lg px-3 py-2">
              <span className="font-mono text-sm tracking-widest text-ink-100 select-all flex-1">
                {newCode}
              </span>
              <button
                onClick={handleCopy}
